 ...
 * @LastEditTime: 2023-04-09 12:23:57
 * @Description: 
...
class UKF {
public:
  UKF(std::string smodel, int state_n, int mea_n, Eigen::MatrixXd Q,
  ...
  void Update(std::vector<Eigen::VectorXd> &Z, const Eigen::VectorXd &beta,
              const float &last_beta);

  void Update(Eigen::VectorXd &Z);

  void Process(Eigen::VectorXd &X, std::vector<Eigen::VectorXd> &Z,
...
private:
  ...
  std::unordered_map<std::string, int> model_hash_ = {
      {"CV", 1}, {"CTRV", 2}, {"CTRA", 3}}; //模型种类字典
  int model_ = 1;                           //模型类别编号
