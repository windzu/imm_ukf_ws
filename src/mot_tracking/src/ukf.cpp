...
float normalangle(float angle) {
  const auto times = std::round(std::fabs(
      angle / (2. * M_PI))); // for the case when angle is very very large
...
bool UKF::Isinitalized() { return is_initalized_; }
  ...
  for (int i = 0; i < 2 * n_x_ + 1; ++i) {
    ...
    if (model_ == 1) { // CV
      ...
      if (fabs(yawd_k) > 0.001) {
        px_pre =
            px_k + velo_k / yawd_k *
                       (std::sin(yaw_k + yawd_k * deltat) - std::sin(yaw_k));
        py_pre =
            py_k + velo_k / yawd_k *
                       (std::cos(yaw_k) - std::cos(yaw_k + yawd_k * deltat));
      } else {
        px_pre = px_k + velo_k * deltat * cos(yaw_k);
        py_pre = py_k + velo_k * deltat * sin(yaw_k);
      }
      ...
      if (fabs(yawd_k) > 0.001) {
        ...
        px_pre =
            px_k + (velo_k * deltat + 0.5 * a_k * deltat * deltat) * cos(yaw_k);
        py_pre =
            py_k + (velo_k * deltat + 0.5 * a_k * deltat * deltat) * sin(yaw_k);
      }
...
void UKF::PredictionZ(Eigen::VectorXd &X, Eigen::MatrixXd &P, float ntime) {
  ...
  // std::cout<<"############################## UKFPrediction END
  // ###################################"<<std::endl;
}

void UKF::Update(std::vector<Eigen::VectorXd> &Z, const Eigen::VectorXd &beta,
                 const float &last_beta) {
  // std::cout<<"############################## UKFUPdate START
  // #######################################"<<std::endl;
  ...
  for (int i = 0; i < Z.size() + 1; ++i) {
    ...
    P_temp = P_temp +
             beta(i) * (a * a.transpose() - x_filter * x_filter.transpose());
  }
  ...
  // std::cout<<"\n ########### end ###########\n"<<x_<<'\n'<<std::endl;
  // std::cout<<"############################## UKFUPdate END
  // #################################"<<std::endl;
}

void UKF::Update(Eigen::VectorXd &Z) {
  // std::cout<<"############################## UKFUPdate NORMAL START
  // #######################################"<<std::endl;
  // std::cout<<"Update"<<std::endl;
  //在这里Ｔ就是关于ｘ和Ｙ的协方差Ｐｘy
  Eigen::MatrixXd T = Eigen::MatrixXd(n_x_, n_z_);
  ...
  // std::cout<<"\n ########### end ###########\n"<<x_<<'\n'<<std::endl;
  // std::cout<<"############################## UKFUPdate NORMAL END
  // #################################"<<std::endl;
}

void UKF::Process(Eigen::VectorXd &X, std::vector<Eigen::VectorXd> &Z,
                  const Eigen::VectorXd &beta, const float &last_beta,
                  Eigen::MatrixXd &P, float time) {
  //初始化
...
Eigen::VectorXd UKF::Get_state() { return x_; }

Eigen::MatrixXd UKF::Get_covariance() { return P_; }

Eigen::VectorXd UKF::Get_Zminus() { return Zminus_; }

Eigen::MatrixXd UKF::Get_S() { return S_; }

Eigen::VectorXd UKF::Get_PredictionZ() { return z_pre_; }
