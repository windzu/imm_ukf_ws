...
sort_points(std::vector<cv::Point2f> &intersectingRegion) {
  ...
  for (int i = 0; i < intersectingRegion.size(); i++) {
    arctan.push_back(
        atan2(intersectingRegion[i].x - intersectingRegion[leftmost_index].x,
              intersectingRegion[i].y - intersectingRegion[leftmost_index].y));
  }
...
inline float RectIou(const cv::RotatedRect &r1, const cv::RotatedRect &r2) {
  ...
  if (intersectingRegion.empty()) {
    ...
    std::vector<cv::Point2f> sort_intersectingRegion =
        sort_points(intersectingRegion);
  ...
  // cout<<"### intersetcion "<<inter_area<<" "<<area1<<" "<<area2<<"
  // "<<iou<<endl;
  return 1 - iou;
};
