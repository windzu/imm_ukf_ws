...
#include "cal_iou.h"
using namespace std;
inline float euclideanDist(const Eigen::VectorXd &_p1,
                           const Eigen::VectorXd &_p2) {
  return sqrt((_p1(0) - _p2(0)) * (_p1(0) - _p2(0)) +
 ...
 */
void clusterq(const cv::Mat &cost_matrix,
              std::vector<std::vector<int>> &cltrack,
              std::vector<std::vector<int>> &dettrack) {
  ...
  while (total_tracks < num_tracks) {
    ...
    std::set_difference(all_tracks.begin(), all_tracks.end(),
                        all_visited_tracks.begin(), all_visited_tracks.end(),
                        std::insert_iterator<std::set<int>>(
                            potential_track, potential_track.begin()));
    ...
    std::set<int> temp;
    std::set_union(all_visited_tracks.begin(), all_visited_tracks.end(),
                   visited_tracks.begin(), visited_tracks.end(),
                   std::insert_iterator<std::set<int>>(temp, temp.begin()));
...
void Tracker::Init(const Detection &detections, float &time) {
  ...
  if (!init_) { //表示第一帧存下的检测结果
    ...
    // std::cout<<"#################### TRACKER start_tracking_ INIT
    // ####################"<<std::endl;
    not_associated_.clear();
    not_associated_.swap(not_associated_);
    // cout<<"after not_associated_.capacity():
    // "<<not_associated_.capacity()<<endl;
...
void Tracker::track(const Detection &detections, float &time,
  ...
  if (!init_ || !start_tracking_) {
    ...
    // std::cout<<"#################### TRACKER tracking
    // #####################"<<std::endl;
    not_associated_.clear();
    not_associated_.swap(not_associated_);
    //级联关联
    // std::cout<<"tracksstart "<<tracks_.size()<<" confirmed_tracks_
    // "<<confirmed_tracks_.size()<<std::endl;
    ...
    cv::Mat_<int> q(cv::Size(confirmed_tracks_.size(), detections.size()),
                    int(0)); //基本关联矩阵q
    ...
    if (q.total() == 0) {
      ...
      for (int i = 0; i < clustersize; ++i) {
        ...
        // q(cv::Rect(colmin, rowmin, colsize, rowsize)).copyTo(x(cv::Rect(1, 0,
        // colsize, rowsize))); std::cout<<"########## q
        // ##########\n"<<q(cv::Rect(colmin, rowmin, colsize,
        // rowsize)).clone()<<std::endl; std::cout<<"########## x
        ...
        const Matrices &association_matrices =
            generate_hypothesis(x); //生成假设矩阵
        // std::cout<<"########## association_matrices ##########\n"<<std::endl;

        Eigen::MatrixXd beta = joint_probability(association_matrices,
                                                 selectdets, tracks); // JPDAF
        // std::cout<<"########## BETA part ##########\n"<<beta<<std::endl;
        int b = 0;
    ...
    if(q.total()==0){
            ...
            const Matrices& association_matrices =
    generate_hypothesis(q);//生成假设矩阵

            Eigen::MatrixXd beta = joint_probability(association_matrices,
    selected_detections);//JPDAF
            ...
            std::cout<<"MeaisAsso "<<MeaisAsso[i]<<std::endl;
                    if(MeaisAsso[i]){
                            ...
                            track->Update(Zv, beta.col(i), beta(beta.rows() - 1,
    i), time);//TODO change the imm_ukf }else{ track->MarkMissed();
                    }
                    ++i;
            }
    }*/
    ...
    for (auto &tr : tracks_) {
      ...
      Box tempb = tr->GetBox();
      save << id, x(0), x(1), x(2), p(0), p(1), tempb.yaw, tempb.length,
          tempb.width, tempb.height, tempb.z;
    ...
    if (unconfirmed_tracks_.size() > 0) {
      ...
      not_associated_.clear();
      not_associated_.swap(not_associated_);
      // std::cout<<"######## not asso clear
      // ###############"<<not_associated_.size()<<std::endl;

      for (const auto &track : unconfirmed_tracks_) {
        // std::cout<<"########################## unconfirm preditcion
        // ########################"<<std::endl;
        track->Prediction(time);
      }

      const uint &UconfirmTrackSize =
          unconfirmed_tracks_
              .size(); //这里指的上一时刻那些没有匹配或是没有形成track的目标
      const uint &detSize =
          seconda_sso_detections.size(); //当前时刻没有匹配的目标

      cv::Mat assigmentsBin =
          cv::Mat::zeros(cv::Size(detSize, UconfirmTrackSize), CV_32SC1);
      cv::Mat costMat = cv::Mat(cv::Size(detSize, UconfirmTrackSize),
                                CV_32FC1); // cosmatrix (cols rows)
      cv::Mat IoucostMat = cv::Mat(cv::Size(detSize, UconfirmTrackSize),
                                   CV_32FC1); // cosmatrix (cols rows)
      ...
      for (uint i = 0; i < UconfirmTrackSize; ++i) {
        for (uint j = 0; j < detSize; ++j) {
          costs.at(i + j * UconfirmTrackSize) =
              euclideanDist(seconda_sso_detections[j].position,
                            unconfirmed_tracks_[i]->GetZ());
          // ioucosts.at(i + j * prevDetSize ) =
          // RectIou(not_associated_.at(j).rotbox,
          // prev_detections_.at(i).rotbox);
      ...
      APS.Solve(costs, UconfirmTrackSize, detSize, assignments,
                AssignmentProblemSolver::optimal);

      const uint &assSize =
          assignments
              .size(); //这个的大小应该是检测结果的大小，里边对应的是目标的编号
      for (uint i = 0; i < assSize; ++i) {
        if (assignments[i] != -1 &&
            costMat.at<float>(i, assignments[i]) < param_.pdist_thresh) {
      ...
      for (uint i = 0; i < rows; ++i) {
        for (uint j = 0; j < cols; ++j) {
          if (assigmentsBin.at<int>(i, j)) {
            unconfirmed_tracks_[i]->Update(
                seconda_sso_detections[j].position); // TODO change the imm_ukf
            unconfirmed_tracks_[i]->UpdateBox(seconda_sso_detections[j]);
            unconfirmed_tracks_[i]->UpdateMeasure(
                seconda_sso_detections[j].position(0),
                seconda_sso_detections[j].position(1));
            trackst[i] = true;
            detst[j] = true;
          }
        }
...
void Tracker::associate(Detection &_selected_detections, cv::Mat &_q,
  ...
  // std::cout<<"#################### TRACKER ASSOCISTATE
  // #########################"<<std::endl; Extracting the measurements inside
  // the validation gate for all the tracks Create a q matrix with a width =
  // clutter + number of tracks
  _q = cv::Mat_<int>(cv::Size(confirmed_tracks_.size() + 1, _detections.size()),
                     int(0));
  ...
  for (const auto &detection : _detections) {
    ...
    for (auto &track : confirmed_tracks_) {
      ...
      const Eigen::MatrixXd &Sin =
          track->S().inverse(); // TODO GET MEASURE COVARIANCE
      cv::Mat S_cv;
      ...
      // "<<mah<<" "<<eucl<<" "<<(param_.pi * param_.pg_sigma *
      // std::sqrt(fabs(Sdt)))<<std::endl;
      if (std::isnan(mah) || std::isnan(eucl) ||
          std::isnan((param_.pi * param_.pg_sigma * std::sqrt(fabs(Sdt)))))
        std::abort();
...
Tracker::Matrices Tracker::generate_hypothesis(const cv::Mat &_q) {
  ...
  uint hyp_num = 0;
  // Generating all the possible association matrices from the possible
  // measurements
  ...
  std::copy(tmp_association_matrices.begin(),
...
//计算beta
Eigen::MatrixXd Tracker::joint_probability(
    const Matrices &_association_matrices, const Detection &selected_detections,
    std::vector<track_ptr> &tracks_in) // TODO CHANGE THE FORM OF DETECTION
{

  // std::cout<<"#################### JPDAF START #########################
  // "<<std::endl;
  ...
  for (uint i = 0; i < hyp_num; ++i) {
    ...
    for (uint j = 0; j < validationIdx; ++j) {
      ...
      const Eigen::MatrixXd &A_matrix =
          _association_matrices.at(i).block(j, 1, 1, tracksize);
      ...
      if (mea_indicator == 1) {
        ...
        for (uint notZero = 0; notZero < tracksize; ++notZero) {
          if (A_matrix(0, notZero) == 1) {
            const Eigen::VectorXd &z_predict =
                tracks_in.at(notZero)->GetZ();                     // TODO!!!!
            const Eigen::MatrixXd &S = tracks_in.at(notZero)->S(); // TODO!!!!
            const Eigen::VectorXd &diff =
                selected_detections.at(j).position - z_predict;
            ...
            N = N / sqrt((2 * CV_PI * S).determinant()) *
                exp(-b); //正态分布
    ...
    if (param_.pd == 1) { // pd代表检测概率，是一个设定值
      ...
      for (uint j = 0; j < tracksize; ++j) {
        const Eigen::MatrixXd &target_matrix =
            _association_matrices.at(i).col(j + 1);
        const int &target_indicator = target_matrix.sum();
        prior = prior * std::pow(param_.pd, target_indicator) *
                std::pow((1 - param_.pd), (1 - target_indicator)); //二项分布
      }
    }
    ...
    for (uint j = 0; j < validationIdx; ++j) {
      ...
      if (mea_indicator == 1) {
        ...
        for (uint notZero = 0; notZero < tracksize; ++notZero) {
          if (A_matrix(0, notZero) == 1) {
            const Eigen::VectorXd &z_predict =
                confirmed_tracks_.at(notZero)->GetZ(); // TODO!!!!
            const Eigen::MatrixXd &S =
                confirmed_tracks_.at(notZero)->S(); // TODO!!!!
            const Eigen::VectorXd &diff =
                selected_detections.at(j).position - z_predict;
            // std::cout<<"########## JPDAF PREDICT ED ##########\n
            // "<<z_predict<<"\n########## S ########\n "<<S<<"\n########## diff
...
void Tracker::manage_tracks(float &time) {
  const uint &prevDetSize =
      prev_detections_.size(); //这里是上一帧中没有匹配上的检测目标
  ...
  if (prevDetSize == 0) {
    ...
    // std::cout<<"######## manage track not asso
    // ###############"<<not_associated_.size()<<std::endl;

    cv::Mat assigmentsBin =
        cv::Mat::zeros(cv::Size(deteSize, prevDetSize), CV_32SC1);
    cv::Mat costMat =
        cv::Mat(cv::Size(deteSize, prevDetSize), CV_32FC1); // cosmatrix
    cv::Mat IoucostMat =
        cv::Mat(cv::Size(deteSize, prevDetSize), CV_32FC1); // cosmatrix
    ...
    for (uint i = 0; i < prevDetSize; ++i) {
      for (uint j = 0; j < deteSize; ++j) {
        costs.at(i + j * prevDetSize) = euclideanDist(
            not_associated_.at(j).position, prev_detections_.at(i).position);
        ioucosts.at(i + j * prevDetSize) = RectIou(
            not_associated_.at(j).rotbox, prev_detections_.at(i).rotbox);
    ...
    AssignmentProblemSolver APS; //匈牙利算法
    APS.Solve(costs, prevDetSize, deteSize, assignments,
              AssignmentProblemSolver::optimal);
    ...
    for (uint i = 0; i < rows; ++i) {
      for (uint j = 0; j < cols; ++j) {
        if (assigmentsBin.at<int>(i, j)) {
          // std::cout<<time<<" "<<pretime<<"
          // "<<(float)(time-pretime)<<std::endl;
          const float &vx = (not_associated_.at(j).position(0) -
                             prev_detections_.at(i).position(0)) /
                            (float)(time - pretime);
          const float &vy = (not_associated_.at(j).position(1) -
                             prev_detections_.at(i).position(1)) /
                            (float)(time - pretime);
          float velo = std::sqrt(vx * vx + vy * vy);
          float angle = atan2(vy, vx);
          // std::cout<<vx<<" "<<vy<<" "<<angle<<std::endl;
          std::shared_ptr<Track> tr(new Track(param_, track_id_, time,
                                              not_associated_.at(j), velo,
                                              angle)); //跟踪初始化 TODO
          track_id_++;
    ...
    cv::Mat notAssignedDet(cv::Size(assigmentsBin.cols, 1), CV_32SC1,
                           cv::Scalar(0));
    ...
    for (uint i = 0; i < dets.total(); ++i) {
      ...
      prev_detections_.push_back(
          not_associated_.at(i)); //把当前帧未匹配的检测存起来留至下一帧匹配
    }
  }
}
...
void Tracker::delete_tracks() {
  for (int i = tracks_.size() - 1; i >= 0; --i) {
    // std::cout<<"############################# delete tracks
    // ###############################"<<tracks_[i]->Age()<<std::endl;
...
void Tracker::pruning(Detection &selected_detections,
  ...
  cv::Mat assigmentsBin =
      cv::Mat::zeros(cv::Size(detSize, TrackSize), CV_32SC1);
  cv::Mat costMat =
      cv::Mat(cv::Size(detSize, TrackSize), CV_32FC1); // cosmatrix (cols rows)
  std::vector<int> assignments;
  std::vector<float> costs(detSize * TrackSize);

  for (uint i = 0; i < TrackSize; ++i) {
    for (uint j = 0; j < detSize; ++j) {
      costs.at(i + j * TrackSize) = euclideanDist(
          selected_detections[j].position, tracks_[i]->GetState());
  ...
  // \n"<<std::endl;
  AssignmentProblemSolver APS; //匈牙利算法
  APS.Solve(costs, TrackSize, detSize, assignments,
            AssignmentProblemSolver::optimal);
  ...
  for (uint i = 0; i < rows; ++i) {
    for (uint j = 0; j < cols; ++j) {
      if (assigmentsBin.at<int>(i, j)) {
        ...
        tracks_[i]->UpdateMeasure(selected_detections[j].position(0),
                                  selected_detections[j].position(1));
        choosen[j] = true;
      }
    }
  }
