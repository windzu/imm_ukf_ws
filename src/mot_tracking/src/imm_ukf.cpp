...
void IMM_UKF::IMM_Initialization(Eigen::VectorXd &Z, float time, float velo,
                                 float angle) {
  // TODO initial x_merge_ interact_pro_
  isinitialized = true;

  filewrite.open("/home/wx/Desktop/tracking_code/trackernew/result.txt",
...
void IMM_UKF::InputInteract() {
  ...
  for (int j = 0; j < model_size; ++j) {
    ...
    for (int i = 0; i < model_size; ++i) {
      float u = (interact_pro_(i, j) * model_pro_(i)) / c_(j);
      P_hat_[j] +=
          (u * (model_P_[i] + (model_X_[i] - X_hat_[j]) *
                                  (model_X_[i] - X_hat_[j]).transpose()));
    }
  }
}
...
void IMM_UKF::PredictionZmerge(float time) {
  ...
  // std::cout<<"############################## IMM Prediction2
  // #############################"<<std::endl;

  for (int i = 0; i < model_size; ++i) {
    Eigen::MatrixXd S = imm_ukf_[i].Get_S();
    Eigen::VectorXd Zp = imm_ukf_[i].Get_PredictionZ();
    S_merge_ += (model_pro_(i) *
                 (S + (Zp - Zpre_merge_) * (Zp - Zpre_merge_).transpose()));
  }

  // std::cout<<"########## Zpre_merge_ ##########\n"<<Zpre_merge_<<"\n
  // ########## Spre_merge_ ##########\n"<<S_merge_<<"\n"<<std::endl;

  // std::cout<<"#################### IMM Prediction END
  // #############################"<<std::endl;
}
...
void IMM_UKF::UpdateProbability(std::vector<Eigen::VectorXd> &Z,
                                const Eigen::VectorXd &beta,
                                const float &last_beta) {
  // std::cout<<"#################### IMM UpdateProbability
  // #############################"<<std::endl;
  ...
  for (int i = 0; i < model_size; ++i) {
    ...
    float ita = 1 / (sqrt(2 * pi_) * sqrt(fabs(S.determinant()))) *
                exp(-0.5 * Zminus.transpose() * S.inverse() * Zminus);
    model_ita[i] = ita;
  }
  //更新IMM算法中的模型匹配
  float c_temp = 0;
  ...
  // "<<model_pro_<<std::endl; std::cout<<"############################## IMM
  // UpdateProbability END #############################"<<std::endl;
}
...
void IMM_UKF::MixProbability() {
  ...
  for (int i = 0; i < model_size; ++i) {
    p_merge_ += model_pro_(i) *
                (model_P_[i] + (model_X_[i] - x_merge_) *
                                   (model_X_[i] - x_merge_).transpose());
  }
  filewrite << track_id_ << " " << x_merge_(0) << " " << x_merge_(1) << " "
            << x_merge_(2) << " " << x_merge_(3) << " " << model_pro_(0) << " "
            << model_pro_(1) << " " << model_pro_(2) << "\n";
  // std::cout<<"final merge P\n"<< p_merge_<<std::endl;
}

void IMM_UKF::Process(std::vector<Eigen::VectorXd> &Z,
                      const Eigen::VectorXd &beta, const float &last_beta,
                      float &time) {
  // std::cout<<"#################### IMM Process
  // ####################"<<std::endl;
  if (!IsInitialized()) {
...
Eigen::VectorXd IMM_UKF::getMixState() { return x_merge_; }

Eigen::MatrixXd IMM_UKF::getMixCovariance() { return p_merge_; }

Eigen::MatrixXd IMM_UKF::GetS() { return S_merge_; }

Eigen::VectorXd IMM_UKF::GetZpre() { return Zpre_merge_; }
