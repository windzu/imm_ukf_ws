 ...
 * @LastEditTime: 2023-04-09 14:13:30
 * @Description:
...
struct Detect {
  ...
  std::vector<float>
      box2D; // 2D box in camera left top point and right down point
...
public:
  Track(Param &p, int id, float time, Detect &det, float velocity, float angle)
    ...
    imm_ukf_ = std::shared_ptr<IMM_UKF>(
        new IMM_UKF(id, param_.pmodel, param_.pstate_v, param_.pmea_v,
                    param_.pP, param_.pQ, param_.pR, param_.pinteract));
  ...
  int Update(std::vector<Eigen::VectorXd> &det, const Eigen::VectorXd &beta,
             const float &last_beta, float &time) {
  ...
  Box GetBox() { return boundingbox; }

  Eigen::Vector2f GetMeasure() { return measure_; }
