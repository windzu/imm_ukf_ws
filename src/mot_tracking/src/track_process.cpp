 ...
 * @LastEditTime: 2023-04-09 12:25:27
 * @Description:
...
inline void draw3dbox(Detect &det, cv::Mat &image, vector<int> &color) {
  ...
  for (int i = 0; i < 8; i++) {
    Eigen::Vector3d point_3d(
        xAxisP[i], yAxisP[i],
        zAxisP[i]); //以３ｄ包围框中心为原点的局部坐标系坐标
    Eigen::Vector3d rotationPoint_3d =
        BoxRotation * point_3d + translation; //转换为激光雷达坐标系下的坐标
...
void track_processor::process(const mot_tracking::detect::ConstPtr &msg) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    object.rotbox = cv::RotatedRect(
        cv::Point2f((v(0) + 25) * 608 / 50, v(1) * 608 / 50),
        cv::Size2f(object.box[1] * 608 / 50, object.box[2] * 608 / 50),
        object.yaw);
  ...
  for (int i = 0; i < result.size(); ++i) {
    Eigen::VectorXd r =
        result[i]; ////id, fx, fy, angle, mx, my, yaw, l, w, h, z
    if (frame != 0) {
    ...
    cout << "det: " << det.position(0) << " " << det.position(1) << " " << det.z
         << " " << det.box[0] << " " << det.box[1] << " " << det.box[2] << endl;
    ...
    [frame id class 截断系数　遮挡系数　观测角　ｘ1 y1 x2 y2 h w l x y z yaw
    score]
    */
    ...
    out_txt_file << setprecision(6) << frame << " " << int(r(0)) << " "
                 << "Car"
                 << " " << 0 << " " << 0 << " " << r(3) << " " << 349.22 << " "
                 << 114.118 << " " << 379.46 << " " << 194.975 << " " << r(9)
                 << " " << r(8) << " " << r(7) << " " << det.position(0) << " "
  ...
  cout << "marker_id: " << marker_id << "max_marker_size_ " << max_marker_size_
       << endl;
...
track_processor::track_processor() {
  ...
  //初始化输出文件
  // out_txt_file.open("/mnt/data/workspace/ROS_MOT/track_ws/src/mot_tracking/result/0002.txt",
  // ios::out|ios::trunc);
