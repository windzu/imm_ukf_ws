 ...
 * @Date: 2023-04-09 12:20:53
 * @LastEditors: windzu windzu1@gmail.com
 * @LastEditTime: 2023-04-09 23:23:09
 * @Description:
...
bool Load_Sensor_Data_Path(std::vector<std::string> &lidarfile_name,
                           std::vector<std::string> &imagefile_name,
...
cv::Point cloud2camera(Eigen::Vector3d input) {
  ...
  RT_velo_to_cam << 7.49916597e-03, -9.99971248e-01, -8.65110297e-04,
      -6.71807577e-03, 1.18652889e-02, 9.54520517e-04, -9.99910318e-01,
      -7.33152811e-02, 9.99882833e-01, 7.49141178e-03, 1.18719929e-02,
      -2.78557062e-01, 0, 0, 0, 1;
  R_rect << 0.99992475, 0.00975976, -0.00734152, 0, -0.0097913, 0.99994262,
      -0.00430371, 0, 0.00729911, 0.0043753, 0.99996319, 0, 0, 0, 0, 1;
  project_matrix << 7.215377e+02, 0.000000e+00, 6.095593e+02, 4.485728e+01,
      0.000000e+00, 7.215377e+02, 1.728540e+02, 2.163791e-01, 0.000000e+00,
...
void draw3dbox(Detect &det, cv::Mat &image, vector<int> &color) {
  ...
  double boxroation[9] = {cos(yaw), -sin(yaw), 0, sin(yaw), cos(yaw),
                          0,        0,         0, 1};

  Eigen::MatrixXd BoxRotation =
      Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(boxroation);
  double xAxisP[8] = {l / 2, l / 2, -l / 2, -l / 2,
                      l / 2, l / 2, -l / 2, -l / 2};
  double yAxisP[8] = {w / 2, -w / 2, -w / 2, w / 2,
                      w / 2, -w / 2, -w / 2, w / 2};
  ...
  cv::line(image, imagepoint[0], imagepoint[1], cv::Scalar(226, 43, 138), 2,
           cv::LINE_AA);
  cv::line(image, imagepoint[1], imagepoint[2], cv::Scalar(r, g, b), 2,
           cv::LINE_AA);
  cv::line(image, imagepoint[2], imagepoint[3], cv::Scalar(r, g, b), 2,
           cv::LINE_AA);
  cv::line(image, imagepoint[3], imagepoint[0], cv::Scalar(r, g, b), 2,
           cv::LINE_AA);

  cv::line(image, imagepoint[4], imagepoint[5], cv::Scalar(226, 43, 138), 2,
           cv::LINE_AA);
  cv::line(image, imagepoint[5], imagepoint[6], cv::Scalar(r, g, b), 2,
           cv::LINE_AA);
  cv::line(image, imagepoint[6], imagepoint[7], cv::Scalar(r, g, b), 2,
           cv::LINE_AA);
  cv::line(image, imagepoint[7], imagepoint[4], cv::Scalar(r, g, b), 2,
           cv::LINE_AA);

  cv::line(image, imagepoint[0], imagepoint[4], cv::Scalar(226, 43, 138), 2,
           cv::LINE_AA);
  cv::line(image, imagepoint[1], imagepoint[5], cv::Scalar(226, 43, 138), 2,
           cv::LINE_AA);
  cv::line(image, imagepoint[2], imagepoint[6], cv::Scalar(r, g, b), 2,
           cv::LINE_AA);
  cv::line(image, imagepoint[3], imagepoint[7], cv::Scalar(r, g, b), 2,
           cv::LINE_AA);

  cv::line(image, imagepoint[0], imagepoint[5], cv::Scalar(226, 43, 138), 2,
           cv::LINE_AA);
  cv::line(image, imagepoint[1], imagepoint[4], cv::Scalar(226, 43, 138), 2,
           cv::LINE_AA);
...
bool LoadPointcloud(pcl::PointCloud<pcl::PointXYZI> &cloud_IN, string path) {
  ...
  if (!inputfile) {
    cerr << "ERROR: Cannot open file " << lidar_filename_path << "! Aborting..."
         << endl;
    return false;
  }
...
int main(int argc, char **argv) {
  ...
  ros::Publisher publidar =
      nh.advertise<sensor_msgs::PointCloud2>("/mot_tracking/pointcloud", 1);
  ros::Publisher pubmarker =
      nh.advertise<visualization_msgs::MarkerArray>("/mot_tracking/box", 1);
  ros::Publisher pubtextmarker =
      nh.advertise<visualization_msgs::MarkerArray>("/mot_tracking/id", 1);
  ...
  //创建结果存放文件夹
  boost::filesystem::path result_dir_path =
      boost::filesystem::path(path) / folder_lidar;
  ...
  for (int i = 0; i < labeldata.size(); ++i) {
    ...
    if (truncation > max_truncation || occlusion > max_occlusion ||
        classname2id[type] != classname2id[trackclass])
      continue;
  ...
  while (ros::ok() && frame < maxframe) {
    ...
    if (frame == 0) {
      ...
      // 如果不是第一帧,则依靠上一帧的状态和当前帧的状态计算出两个状态之间的变换矩阵
      // translate2origion ： 当前帧状态到第一帧状态的变换矩阵
      // origion2translate ： 第一帧状态到当前帧状态的变换矩阵
      // twosub ： 第一帧状态到当前帧的朝向角度差
    ...
    for (int i = 0; i < size; ++i) {
      ...
      cout << "input: " << Inputdets[frame][i].position(0) << " "
           << Inputdets[frame][i].position(1) << " " << Inputdets[frame][i].z
           << " " << Inputdets[frame][i].box[0] << " "
           << Inputdets[frame][i].box[1] << " " << Inputdets[frame][i].box[2]
           << endl;

      // lidar position from lidar to map
      // position 平移变换
      // 计算当前帧的目标在第一帧中的位置(计算局部坐标)
      ...
      // 以第一帧的状态为基准,更新当前帧的位置和朝向
      Inputdets[frame][i].position(0) = v(0);
      Inputdets[frame][i].position(1) = v(1);

      Inputdets[frame][i].rotbox =
          cv::RotatedRect(cv::Point2f((v(0) + 25) * 608 / 50, v(1) * 608 / 50),
                          cv::Size2f(Inputdets[frame][i].box[1] * 608 / 50,
                                     Inputdets[frame][i].box[2] * 608 / 50),
                          Inputdets[frame][i].yaw);

      cv::RotatedRect detshow =
          cv::RotatedRect(cv::Point2f((v(0) + 25) * 608 / 50, v(1) * 608 / 50),
                          cv::Size2f(Inputdets[frame][i].box[1] * 608 / 50,
                                     Inputdets[frame][i].box[2] * 608 / 50),
                          Inputdets[frame][i].yaw);
      ...
      for (int j = 0; j < 4; j++)
        cv::line(images, vertices[j], vertices[(j + 1) % 4],
                 cv::Scalar(0, 0, 255), 1);
    }
    ...
    // debug
    for (int i = 0; i < Inputdets[frame].size(); i++) {
      Detect det = Inputdets[frame][i];
      // debug
      std::cout << "input det : " << std::endl;
      std::cout << "    classname: " << det.classname << std::endl;
      std::cout << "    yaw: " << det.yaw << std::endl;
      std::cout << "    position: " << det.position(0) << " " << det.position(1)
                << std::endl;
      std::cout << "    box: " << det.box[0] << " " << det.box[1] << " "
                << det.box[2] << std::endl;
      std::cout << "    rotbox: " << det.rotbox.center.x << " "
                << det.rotbox.center.y << " " << det.rotbox.size.width << " "
                << det.rotbox.size.height << " " << det.rotbox.angle
                << std::endl;
    }
    ...
    for (int i = 0; i < result.size(); ++i) {
      ...
      cout << "det: " << r(0) << " " << det.position(0) << " "
           << det.position(1) << " " << det.z << " " << det.box[0] << " "
           << det.box[1] << " " << det.box[2] << endl;
    ...
    cout << "marker_id: " << marker_id << "max_marker_size_ "
         << max_marker_size_ << endl;
    ...
    sensor_msgs::ImagePtr imgmsg =
        cv_bridge::CvImage(std_msgs::Header(), "bgr8", rgbimage)
            .toImageMsg(); //转换为ros接受的消息
    pubimage.publish(imgmsg);

    sensor_msgs::ImagePtr imgmsg_ori =
        cv_bridge::CvImage(std_msgs::Header(), "bgr8", rgbimage_ori)
            .toImageMsg(); //转换为ros接受的消息
